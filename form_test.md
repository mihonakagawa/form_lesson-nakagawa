...
1. formタグに必要な二つの属性とはなんですか？
  - action, method
  - action...送信先を指定するためのも
  - method...フォームデータの送信方法を設定するための属性で、GET または POST で指定します。
...
3. name属性はどのような役割がありますか？
  ...
  <!--
  また、同じnameが複数存在する場合、 要素の種類や属性によって異なる効果がある。
  今回はラジオボタンのところで同nameが複数存在していたため
  (name="gender")、複数の選択肢の中から1つだけを選択できるようになっていた。
  -->
...
7. inputタグやselectタグに`appearance: none;`を指定するとどうなりますか？また、なぜ設定する必要があるのですか？
  ...
  ※ユーザーが利用しているブラウザのことをユーザーエージェントとも呼ぶ

8. ベンダープレフィックスとはなんですか？
  - CSSの新しいプロパティに付けて、記述したプロパティが拡張機能であることを、各ブラウザに示すために使われる識別子　接頭辞
  - 例）
